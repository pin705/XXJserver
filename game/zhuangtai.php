...
if ($cmd == 'setzbwz'){
    ...
    if (isset($zbnowid) && isset($zbwz)){
        if (!in_array($zbnowid,$arr)){
            ...
            if ($nowzb->idNguoiDung != $nguoiChoi->idNguoiDung){
                echo "Ngươi không có nên trang bị, không cách nào trang bị<br/>";

            }elseif($nowzb->capDoYeuCau - $nguoiChoi->capDo > 5){
...
if ($nguoiChoi->tool1!=0){
    $zhuangbei = Helpers\layThongTinTrangBiTheoId($nguoiChoi->tool1, $dblj);
...
$tool1 = '<a href="?cmd='.$encode->encode("cmd=chakanzb&zbnowid=$nguoiChoi->tool1&uid=$nguoiChoi->idNguoiDung&sid=$sid").'">
<font color='.$zhuangbei->phamChat.'>'.$zhuangbei->tenTrangBi.'</font>'.$qhs.'</a>'.$xxzb1;
}
if ($nguoiChoi->tool2!=0){
    $zhuangbei = Helpers\layThongTinTrangBiTheoId($nguoiChoi->tool2, $dblj);
    $qhs = '';
    if ($zhuangbei->capCuongHoa>0){
        $qhs = '+'.$zhuangbei->capCuongHoa;
    }
    $xxzb2 = '<a href="?cmd='.$encode->encode("cmd=xxzb&zbwz=2&sid=$sid").'">Dỡ xuống</a>';
    $tool2 = '<a href="?cmd='.$encode->encode("cmd=chakanzb&zbnowid=$nguoiChoi->tool2&uid=$nguoiChoi->idNguoiDung&sid=$sid").'"><font color='.$zhuangbei->phamChat.'>'.$zhuangbei->tenTrangBi.'</font>'.$qhs.'</a>'.$xxzb2;
}
if ($nguoiChoi->tool3!=0){
    $zhuangbei = Helpers\layThongTinTrangBiTheoId($nguoiChoi->tool3, $dblj);
    $qhs = '';
    if ($zhuangbei->capCuongHoa>0){
        $qhs = '+'.$zhuangbei->capCuongHoa;
    }
    $xxzb3 = '<a href="?cmd='.$encode->encode("cmd=xxzb&zbwz=3&sid=$sid").'">Dỡ xuống</a>';
    $tool3 = '<a href="?cmd='.$encode->encode("cmd=chakanzb&zbnowid=$nguoiChoi->tool3&uid=$nguoiChoi->idNguoiDung&sid=$sid").'"><font color='.$zhuangbei->phamChat.'>'.$zhuangbei->tenTrangBi.'</font>'.$qhs.'</a>'.$xxzb3;
}
if ($nguoiChoi->tool4!=0){
    $zhuangbei = Helpers\layThongTinTrangBiTheoId($nguoiChoi->tool4, $dblj);
    $qhs = '';
    if ($zhuangbei->capCuongHoa>0){
        $qhs = '+'.$zhuangbei->capCuongHoa;
    }
    $xxzb4 = '<a href="?cmd='.$encode->encode("cmd=xxzb&zbwz=4&sid=$sid").'">Dỡ xuống</a>';
    $tool4 = '<a href="?cmd='.$encode->encode("cmd=chakanzb&zbnowid=$nguoiChoi->tool4&uid=$nguoiChoi->idNguoiDung&sid=$sid").'"><font color='.$zhuangbei->phamChat.'>'.$zhuangbei->tenTrangBi.'</font>'.$qhs.'</a>'.$xxzb4;
}
if ($nguoiChoi->tool5!=0){
    $zhuangbei = Helpers\layThongTinTrangBiTheoId($nguoiChoi->tool5, $dblj);
    $qhs = '';
    if ($zhuangbei->capCuongHoa>0){
        $qhs = '+'.$zhuangbei->capCuongHoa;
    }
    $xxzb5 = '<a href="?cmd='.$encode->encode("cmd=xxzb&zbwz=5&sid=$sid").'">Dỡ xuống</a>';
    $tool5 = '<a href="?cmd='.$encode->encode("cmd=chakanzb&zbnowid=$nguoiChoi->tool5&uid=$nguoiChoi->idNguoiDung&sid=$sid").'"><font color='.$zhuangbei->phamChat.'>'.$zhuangbei->tenTrangBi.'</font>'.$qhs.'</a>'.$xxzb5;
}
if ($nguoiChoi->tool6!=0){
    $zhuangbei = Helpers\layThongTinTrangBiTheoId($nguoiChoi->tool6, $dblj);
    $qhs = '';
    if ($zhuangbei->capCuongHoa>0){
        $qhs = '+'.$zhuangbei->capCuongHoa;
    }
    $xxzb6 = '<a href="?cmd='.$encode->encode("cmd=xxzb&zbwz=6&sid=$sid").'">Dỡ xuống</a>';
    $tool6 = '<a href="?cmd='.$encode->encode("cmd=chakanzb&zbnowid=$nguoiChoi->tool6&uid=$nguoiChoi->idNguoiDung&sid=$sid").'"><font color='.$zhuangbei->phamChat.'>'.$zhuangbei->tenTrangBi.'</font>'.$qhs.'</a>'.$xxzb6;
...
if ($nguoiChoi->tool7!=0){
    $zhuangbei = Helpers\layThongTinTrangBiTheoId($nguoiChoi->tool7, $dblj);
    $qhs = '';
    if ($zhuangbei->capCuongHoa>0){
        $qhs = '+'.$zhuangbei->capCuongHoa;
    }
    $xxzb7 = '<a href="?cmd='.$encode->encode("cmd=xxzb&zbwz=7&sid=$sid").'">Dỡ xuống</a>';
    $tool7 = '<a href="?cmd='.$encode->encode("cmd=chakanzb&zbnowid=$nguoiChoi->tool7&uid=$nguoiChoi->idNguoiDung&sid=$sid").'"><font color='.$zhuangbei->phamChat.'>'.$zhuangbei->tenTrangBi.'</font>'.$qhs.'</a>'.$xxzb7;
}
...
<IMG width="50" height="50" src="./images/$nguoiChoi->gioiTinh.png"><BR>
Biệt danh:$nguoiChoi->tenNhanVat<br/>
Thân phận:$sf <a href="?cmd=$wbts" style="border-radius: 15px;margin-top: 0px;">!</a>$wenben<br>
Cảnh giới:$nguoiChoi->canhGioi $nguoiChoi->tangCanhGioi<br/>
Đẳng cấp:$nguoiChoi->capDo<br/>
<span style="display: flex;">VIP:<IMG width="30" height="25" src="./images/vi$nguoiChoi->vip.png"></span>
Ma thạch:<font color="#ff0000" size="3.5">$nguoiChoi->tienNap</font><br/>
Linh thạch:$nguoiChoi->tienTroChoi<br/>

Tu vi:$nguoiChoi->kinhNghiem/$nguoiChoi->kinhNghiemToiDa<br/>
Khí huyết:$nguoiChoi->sinhMenh/$nguoiChoi->sinhMenhToiDa<br/>
Công kích:$nguoiChoi->congKich<br>
<!--Sáo trang tăng thêm:{$gj}00%<br/>-->
Phòng ngự:$nguoiChoi->phongNgu<br/>
Bạo kích:$nguoiChoi->baoKich<br/>
Hút máu:$nguoiChoi->hutMau<br/>
Thiên phú:$nguoiChoi->tf<br>
May mắn:$nguoiChoi->thienPhuMayMan<br>
Né tránh:$nguoiChoi->thienPhuNeTranh<br>
<hr>
