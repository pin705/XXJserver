...
if($cmd=='pvb' && $boss->sid==''){
    ...
    if ($player->ulv >= 10 && $player->uhp <=0){
        ...
        $sql = "update boss set sid = '$sid' WHERE id='$bossid'";
        $dblj->exec($sql);
...
elseif ($cmd == 'pvbgj' && $bossid != 0){
   ...
   // }
    ...
    $ran = mt_rand(1,100);//Khống chế bạo kích xác suất
    if ($player->ubj >= $ran){
    ...
    $pvbxx = ceil($gphurt * ($player->uxx/100) );//Khống chế hút máu cường độ
    ...
    if ($boss->bosshp <=0 ){//Quái vật tử vong, ban thưởng tính toán
		...
		//$dblj->exec($sql);
        $yxb = round($boss->bosslv*2.9) + 110;//Tiền trò chơi tính toán
        if ($hurt || $lvc >=5){
            $yxb = 0;//Lớn hơn cấp năm tiền trò chơi là không
        }
...
HTML;
        ...
        if ($boss->dljv >=$sjjv && $boss->bosszb != ''){
			...
			$huode .= "nhận được :".'<a href="?cmd='.$chakanzb.'"><font color='.$zbys->zbys.' >'.$zbname .'</font></a><br>';//Trang bị yếu tố
                    ...
                    $stmt->execute(array('【Phụ trương】',"[{$player->uname}]{$jisha}[{$boss->bossname}]nhận được [{$zbname}]Khiến người ghen tị。",'0'));//Hệ thống thông cáo
					}
            }
        }
		//Đạo cụ tính toán
        ...
        // yp:
        // $sjjv = mt_rand(1,100);
        // if ($boss->ypjv >= $sjjv && $boss->gyp != ''){
            // $sql = "select * from yaopin WHERE ypid in ($boss->gyp)";
            // $cxdljg = $dblj->query($sql);
    ...
    $pzssh = $phurt - $pvbxx;//Lượng biến đổi
...
if (isset($zdjg)){//Nơi này là chiến đấu trải qua, có ba cái 0 1-1 Lựa chọn
    switch ($zdjg){
        case 1:
            ...
            Ngươi đánh chết$boss->bossname<br/>
...
Khí huyết:(<div class="hpys" style="display: inline">$boss->bosshp</div>/<div class="hpys" style="display: inline">$boss->bossmaxhp</div>)$pvbbj$ghurt<br/>
Công kích:($boss->bossgj)<br/>
Phòng ngự:($boss->bossfy)<br/>
Trước mắt công kích nhân số:<br/>
===================<br/>
$player->uname [lv:$player->ulv]<br/>
Khí huyết:(<div class="hpys" style="display: inline">$player->uhp</div>/<div class="hpys" style="display: inline">$player->umaxhp</div>)$phurt$pvbxx<br/>
